...
const ROUTES: RouteUnderTest[] = [
  ...
  { method: 'post', path: '/api/v1/clients', capabilities: ['clients.manage'], body: {} },
  { method: 'get', path: '/api/v1/billing/margin', capabilities: ['billing.read'] },
  { method: 'get', path: '/api/v1/billing/margin/export.csv', capabilities: ['billing.read'] },
...
const OPEN_TO_ANY_SIGNED_IN_USER = new Map<string, string>([
  ...
  ['GET /api/v1/notifications/preferences', 'reads and writes the caller’s own contact details'],
  ['PATCH /api/v1/notifications/preferences', 'reads and writes the caller’s own contact details'],
  ['GET /api/v1/dashboard/summary', 'shaped by the capabilities the caller holds'],
  ['GET /api/v1/auth/mfa', 'their own second factor, and whether their role needs one'],
  ['POST /api/v1/auth/mfa/enrol', 'adds a second factor to the caller’s own account'],
  ['POST /api/v1/auth/mfa/activate', 'turns on the caller’s own second factor'],
  ['DELETE /api/v1/auth/mfa', 'removes the caller’s own, and only where their role allows it'],
